...
두 변수가 종속 -> 강한상관관계 -> 직선관계 -> 한변수처럼 -> 차원축소

why?
1. 차원 높으면 분석시간 증가, 불필요한 
2. 과적합 문제
3. 설명력이 떨어짐
...
- DW: 데이터 창고
- DM: 주제에 필요한 데이터 정리된 창고

### (4) 변수변환: 데이터를 분석에 필요한 형태로 변환
1. 구간화(binning): 연속형 변수를 범주화, 나이 -> 10대 20대
2. 정규화: 독립변수의 단위를 동일하게 하는 방법
- Min-Max Scaling: 대표적인 스케일링 방법, (x-xmin)/(xmax-xmin), 0~1 사이, 이상치 영향 받음
- standardization(z-score): 수치와 평균의 차이를 표준편차로 나눈 값
- robust: 이상치 탐지에 유리, 4분위 편차 이용
3. 더미변수화: 범주형 자료를 0과 1로 변경, 숫자 의미 x
- 통계에서 더미변수, 머신러닝에서 원핫인코딩
6. 함수변환: 로그 지수 역수 루트 변환
- 데이터의 왜도, 첨도를 줄임
- 로그, 루트, 역수 -> 숫자 작아짐 -> positive skew를 정규분포로
- 지수, 제곱 -> 숫자 커짐 -> negative skew를 정규분포로
7. box-cow 변환
- 분포의 skew를 조절
- 정규분포에 가깝게 만드는 람다를 찾기
...
- 랜덤 오버 샘플링: 무작위로 데이터 복제
- SMOTE: 중심이 되는 데이터와 주변 데이터 사이의 가상의 직선 만든 후, 그 위에 데이터 추가
- Borderline-SMOTE: 경계선에서 smote적용
- ADASYN
...
### (2) 상관관계 분석
- 상관계수: 직선 관계가 어느 정도 있는지 측정
- 공분산을 x,y 표준편차로 나눈 수치 (-1~1)
- e=0 직선관계x
- e=1에 가까울수록 상관관계 높다
1. Pearson: 연속형일 경우
2. Spearman: 서열자료일 경우
...
#### 공분산: 두 변수가 증가 증가 또는 감소 감소 경향성
- 평균으로부터 두 변수가 얼마나 같이 떨어져 있는가
- 한계: 이상치 있으면 괴리 생김 -> 상관계수 개념 필요

### (3) 기초통계량 추출 및 이해
#### 중심화 경향: 한 값으로 전체 데이터 중앙을 대표
1. 평균
...
2. 중위수: 중앙값, 정렬했을때 위치가 중앙(50%)에 위치한 값, 극단치 제거
3. 최빈값: 가장 자주 나타나는 값
#### 산포도: 데이터의 중심에서 얼마나 퍼져있는가 측정
1. 범위: 최대값 - 최소값
2. 사분위편차: 정렬된 자료에서 25% ~ 75% 구간 길이 측정
3. 평균편차: (x-abs(x))/n
4. 분산: 편차의 제곱의 평균
5. 변동계수: 두 집단의 상대적 산포의 크기 비교, 클수록 평균에서 멀리 분포
#### 분포도
1. 왜도: 분포의 좌우 비대칭성, 중심이 왼쪽에 있으면 양수 값
- skewness >0 : 평균에 비해 많이 큰 값 존재, Positive, mean>median>mode
2. 첨도: 분포의 뾰족한 정도, 편차 4승으로 계산
- kurtosis > 3 뾰족
- kurtosis = 3 정규분포
- kurtosis < 3 완만
...
#### 범주형 자료 시각적 탐색
1. 원그래프
2. 막대 그래프(bar): 각 데이터 변수에 대한 값
3. 꺾은선 그래프

#### 연속형 자료 시작적 탐색
1. 히스토그램: 각 데이터의 구간 별 도수
2. 상자-수염도표: 중위수, 최대값, 최소값, 사분위수 이용해 표현
3. 도수곡선

#### 두 변수간 관계의 시각적 탐색
1. 산점도: x,y 좌표로 표현, 상관관계
...
### (3) 비정형 데이터 탐색: 이미지, 영상, 텍스트(sns 실시간 대화, 이메일 내용), 로그파일처럼 구조화 되지 않은 데이터
1. 텍스트마이닝
...
## 2.3 통계기법 이해
### (1) 데이터 요약: 자료를 요약,정리함으로써 의미와 가치가 부여됨
#### 통계학의 목적
...
### (2) 구간추정: 오차를 고려해 특정한 하나의 값이 다르게 나타날 수 있는 범위까지 추정
...
### (3) 가설검정: 모집단에 대한 가설을 검정하는 분석방법
#### 귀무가설: 차이가 없다, 효과 없다를 주장   귀없다
- ~없다로 표현
