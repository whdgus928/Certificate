...
#### 피라미드 DIKW
- 지혜: a 이과성향, 과학 잘할 것 -> 축적된 결과로 예측
- 지식: a에서 이과를 많이 감 -> 정보 통해 의미 도출
- 정보: a가 수학 더 잘함 -> 데이터 가공, 의미x
-데이터: a 수학 90, b 수학 80 -> 순수한 사실, 의미x

#### 빅데이터: 대량의 데이터 이상의 정형 또는 비정형 데이터, 가치를 추출하고 분석하는 기술
- 3V: 규모(volume), 속도(Velocity), 다양성(Variety)
- 5v: 가치,신뢰성(Veracity)
- 7v: 휘발성(Volatility),정확성
...
### (4) 빅데이터 조직 및 인력
- 소프트 스킬: 통찰력, 협력 능력, 전달력
- 하드 스킬: 빅데이터 이론 지식, 숙련도

### (1) 빅데이터 플랫폼
데이터 소스 -> 데이터 수집 -> 데이터 저장 처리 관리 -> 데이터 분석 -> 활용

1. 데이터 소스 : sns,음악,로그,erp
2. 데이터 수집
...
3. 데이터 저장 : DBMS, MongoDB, HDFS
4. 데이터 처리: Hadoop(분산처리, 오픈소스), Mapreduce
5. 데이터 분석: 데이터마이닝, 분류, 군집, 머신러닝

빅데이터 에코 시스템: 관리 + 분석 플랫폼
...
### (3) 개인정보 법 제도
개인정보: 개인을 식별할 수 있는 가능성이 있는 데이터

### (4) 개인정보 활용
개인정보 비식별화
- 가명처리
- 총계처리: 통계값을 적용해 특정 개인 판단x
- 데이터 삭제
- 데이터 범주화
- 데이터 마스킹: 대체값으로 변환
...
### (2) 분석 절차 및 작업 계획
Data - ETL(extract, transform, load) process - Meta data(warehouse) - data Marts - uesrs
...
- 조직 내부에 위치
- 수집 용이한 정형
- 서비스: SCM(supply chain Management) 공급관리, ERP(Enterprise Resoruce Planning) 전사적 기업 관리, CRM(Customer Relationship Management) 고객관리
...
3. 스쿱(Sql+hadoop): MYSQL, Oracle 같은 관계형 데이터베이스 시스템에서 하둡 파일 시스템으로 데이터를 수집하거나, 하둡에서 관계형으로 데이터 전송
- 병렬 데이터 전송
...
5. RSYNC: 1:1 서버 클라이언트, 파일과 디렉토리 연결

#### 비정형 데이터 수집 방식 및 기술
스크래파이와 아파치 카프카의 활용 증가 추세
1. 크롤링: 웹 문서 및 콘텐츠 수집 기술
2. RSS: xml 기반, 블로그 뉴스 수집
...
5. 아파치 카프카: 실시간 로그처리하는 레코드 스트림을 발행,구독하는 방식의 분산 스트리밍 플랫폼 기술
- 신뢰성, 확장성 제공
- 주요기능: 소스 채널 싱크 인터프리터
...
5. 척와: 분산 서버에서 에이전트와 컬렉터 구성으로 데이터 수집, 데이터를 하둡 파일 시스템에 저장하는 데이터 수집 기술
...
#### 데이터 속성
수치형 변수: 몇 개, 연산 가능
- 비율 척도: 연산했을 때 의미가 있음, 절대 영점 존재 ex) 스테이크 중량, 몸무게, 키

범주형 변수: 범주로 구분해 측정된 변수, 연산 의미 없음
- 명목 척도: 임의의 범주로 분류한 후 기호나 숫자를 부여, ex) 한우부위분류, 혈액형
- 서열 척도: 서열의 순서화 ex) 지방함유량 순위
- 등간 척도: 간격, 차이, 절대원점x ex)굽기정도, 20도 덥다는 10도의 2배가 아님

### (3) 데이터 변환
1. 평활화: 잡음 제거위해 추세에 벗어나는 값 변환
2. 집계: 그룹화 연산으로 데이터 요약, 월별합계
...
k-익명성: 데이터에서 준식별자 속성들이 동일한 레코드가 적어도 k개 존재
l-다양성: k-익명성 보완, 익명성을 향상, 동질성 공격과 배경 지식에 의한 공격을 방어
t-근접성: 동질 집합, 전체 데이터에서 민감정보의 분포가 유사한 차이를 보이게 하는 모델

### (5) 데이터 품질 검증

### (1) 데이터 적재
1. 플루언티드: 중간에서 트래픽 조절, 중앙로그 저장소로 전송
2. 플럼: 많은 로그 데이터 효율적으로 수집 저장
3. 스크라이브: 로그 데이터 실시간으로 받아오는 서버기술
4. 로그스태시: 많은 로그 데이터를 하나의 저장소로 출력

### (2) 데이터 저장
#### CAP 이론: 분산시스템에 필요한 3개중 2개만 동시에 가능
1. consistency: 분산환경에서 모든 노드가 같은 시점 데이터 show
2. availble: 모든 사용자 읽고 쓰기 가능
3. partion tolerence: 통신 실패에도 정상 작동

1. 구글 파일 시스템
- Fault tolerance: 네트워크, cpm 장애조치가 원만
- scalability: 다수 사용자, 수많은 서버 확장성
- Auto computing: 관리 힘듦 -> 자동할당
- 마스터: 파일메타정보 저장
- 청크서버: 청크 저장, 파일처리
- 클라이언트: 마스터에 청크인덱스 요청

2. NoSQL: BASE 충족
- Basic Available: 부분 고장 있어도 사용가능, 서버고장 -> 백업서버
- soft state: 일관성 없는 상태에서도 data 읽기 가능, 현시점 최신상태
- Eventual Consistency: 시간이 지나면 일관성 유지
- 
